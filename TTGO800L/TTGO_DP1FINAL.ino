...
String savedMobileNumbers[4] = {"63", "63", "63", "63"}; // PINAKA ITETEXT

// GPS ADDRESS VARIABLES
const size_t bufferSize = 4096;
char buffer[bufferSize];
// double lat = 14.5989945;
// double lang = 121.0028021;
const char* apiToken = "128816616938358395746x22401";
String gpsAddress = "";
String lac, cid;
String cellapiToken = "pk.24961857243e24ac571374f55318f86f";
...
ezButton button(12); // create ezButton object that attach to pin GPIO21
      ...
      if (client.available()) {
        ...
        if (c == '\n') {
          if (currentLine.length() == 0) {
            ...
            client.println("</header>");
            // ===============================================================================================
            client.println("<!--For adding USERNAME -->");
            ...
            for (int i = 0; i < 4; i++) {
                ...
                client.print("                     <input type=\"text\" id=\"mobileNumber" + String(i + 1) + "\" name=\"mobileNumber" + String(i + 1) + "\" class=\"inputField\" value=\"" + mobileNumbers[i] + "\" maxlength=\"12\" minlength=\"12\"");
                
                // Only add 'required' attribute if the value is not "63"
...
void saveMobileNumbers(String request, WiFiClient &client) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (equalsIndex != -1) {
      ...
      // Directly set mobileNumbers[i] without additional checks
...
//=========================================================================================
  ...
  SerialMon.println("Making HTTPS request to GeocodeXYZ API...");

  if (client.connect("geocode.xyz", 80)) {
    String load;
    SerialMon.println("Connected to GeocodeXYZ API server");

    // Replace these coordinates with your actual location
    String url = "/" + coordinates + "?geoit=json&auth=" + apiToken;
    //SerialMon.println(url);
    client.println("GET " + url + " HTTP/1.1");
    client.println("Host: geocode.xyz");
    client.println("Connection: close");
    client.println();
    ...
    while (client.available())
    {
      load = client.readString();
    }
    
    delay(5000);

    const size_t bufferSize = JSON_OBJECT_SIZE(10) + 350;

    //StaticJsonDocument<192> doc;
    StaticJsonDocument<bufferSize> doc;
    deserializeJson(doc, load);
     
    const char* streetnm = doc["intersection"]["street2"];
    const char* brgynm = doc["osmtags"]["name"];
    const char* citynm = doc["city"];
    int postalcode = doc["osmtags"]["postal_code"];
   
    SerialMon.println(streetnm);
    SerialMon.println(brgynm);
    SerialMon.println(citynm);
    SerialMon.println(postalcode);

    gpsAddress = "\nCoordinates: " + String(coordinates) + "\nLocation: " + streetnm + ", " + brgynm + ", " + citynm;
    ...
    SerialMon.println("Failed to connect to GeocodeXYZ API server");
  }
}
